...
/*
 * UDP방식 : 비연결 지향, 신뢰성이 없다. 데이터가 순서대로 도착한다는 보징이없다.
	...
	public static void main(String[] args) {
		try {
			...
			while (true) {
				...
				byte[] bMsg = new byte[521];

				// 수신용 패킷객체 생성
				// >> 데이터가 저장될 byte형 배열, 배열의 길이를 생성자의 인수로 지정하여 생성한다.
				inpacket = new DatagramPacket(bMsg, bMsg.length);// 받을 배열하고 , 배열의 갯수

				// 데이터 수신하기 == receive()메서드 이용
				// 이 메서드는 데이터가 올 때 까지 기다린다.
				// 수신된 데이터의 패킷정보는 지정한 패킷객체에 저장된다.
				socket.receive(inpacket); // 수신용 packet을 넣는다.
				...
				// - 실제 읽어온 데이터의 길이는 수신용 패킷객체의 getLength()메서드 통해서 구할 수 있다.

				// 수신된 데이터는 byte형 배열로 오기 때문에 이 데이터를 문자열로 변환해서 출력한다.
				String msg = new String(bMsg, 0, inpacket.getLength(), "utf-8");// 방법1
//				String msg2 = new String(inpacket.getData(),0,inpacket.getLength(),"utf-8");//방법2
