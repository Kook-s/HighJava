...
//접속한 클라이언트 정보를 지정할 Map객체 선언
//	  >> Key값은 '대화명' , value값은 클라이언트와 연결된 소켓 객체
	private Map<String, Socket> clientMap;

	// 생성자
	public TcpMultiChatServer() {
		// clientMap을 동기화 처리가 되로록 생성한다.
...
//	clientMap에 저장된 전체 사용자게에 메시지를 전송하는 메서드
...
//----------------------------------------------------------------------------------------
	...
	class ServerReciver extends Thread {
		...
		public void run() {
			...
			try {
				// 클라이언트가 연결이 성공되면 첫번째로 '대화명'을 입력받아 보낸다.
				// 서버에서는 이 '대화명'을 받아서 중복여부를 검사하여 그 결과를 응답으로 보낸다.

				while (true) {// '대화명'이 중복되지 않을 떄까지 반복한다.
					name = din.readUTF(); // '대화명' 수신

					// 중복 검사
					if (clientMap.containsKey(name)) {// 중복 되면
